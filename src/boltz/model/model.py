...
class Boltz1(LightningModule):
        ...
        with torch.set_grad_enabled(
            ...
            for i in range(recycling_steps + 1):
                with torch.set_grad_enabled(self.training and (i == recycling_steps)):
                    ...
                    s, z, s_l = pairformer_module(s, z, mask=mask, pair_mask=pair_mask,recycle_times=i)

                    s_list.append(s_l)  

            # torch.save(s_list, f'/data/home/luruiqiang/guchunbin/boltz/recycle_test_50.pt')
            # torch.save(s_list, f'/data/home/luruiqiang/guchunbin/boltz/coe_results/{self.predict_args['name']}.pt')
            pdistogram = self.distogram_module(z)
            dict_out = {"pdistogram": pdistogram,"coe": s_list}
            # dict_out.update("coe": s_list)
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        try:
            ...
            pred_dict['coe'] = out["coe"]
            pred_dict['token_a_list'] = out["token_a_list"]
